...
if (pdf.numPages >= 2) {
    ...
    */
    ...
    /*
       PDF PAGE BOUNDARY CHECK — NUTRITION

       Keep each physical PDF page as a separate extraction segment.
       Each Nutrition value is found on the page containing its label,
       so a value at the end of one page cannot consume unrelated text
       from the next page while looking for its terminator.

       This is ONLY the page-boundary check. Field-boundary handling
       for values that genuinely continue across pages will be handled
       separately.
    */
    const nutritionPageTexts = [];

    if (nutritionStartIndex !== -1) {
        nutritionPageTexts.push(
            page1Text.substring(nutritionStartIndex)
        );
        nutritionPageTexts.push(page2Text);
    } else {
        nutritionPageTexts.push(page2Text);
    }

    console.log("=================================");
    console.log("NUTRITION PAGE SEGMENTS — PAGE BOUNDARY CHECK");
    console.log("=================================");
    nutritionPageTexts.forEach((pageText, index) => {
        console.log(`NUTRITION PDF PAGE ${index + 1}:`, pageText);
    });

    page1Data.nutrition =
        extractNutritionData(nutritionText, nutritionText);

    const nutritionPageValue =
        (label, possibleEnds) => {
            for (const pageText of nutritionPageTexts) {
                const normalizedPageText =
                    normalizeNutritionText(pageText);

                if (normalizedPageText.indexOf(label) !== -1) {
    ...
    page1Data.nutrition.servingSize =
        nutritionPageValue("Serving Size:", ["Calories"]);
    page1Data.nutrition.calories =
        nutritionPageValue("Calories", ["Protein"]);
    page1Data.nutrition.protein =
        nutritionPageValue("Protein", ["Carbohydrates"]);
    page1Data.nutrition.carbohydrates =
        nutritionPageValue("Carbohydrates", ["Fiber"]);
    page1Data.nutrition.fiber =
        nutritionPageValue("Fiber", ["Net Carbohydrates"]);
    page1Data.nutrition.netCarbohydrates =
        nutritionPageValue("Net Carbohydrates", ["Healthy Fat"]);
    page1Data.nutrition.healthyFat =
        nutritionPageValue("Healthy Fat", [
